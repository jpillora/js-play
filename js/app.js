...
(function() {
  ...
  App.controller('Spotlight', function($rootScope, $scope, key) {
    ...
    scope.shown = false;
    return key.bind('Super-\\', function() {
  ...
  App.factory('gh', function($http, $rootScope, $timeout, storage, console, datums) {
    ...
    gh.logout = function() {
      storage.del("auth");
      gh.github = null;
      return gh.authed = false;
    };
    getToken = function(code) {
      return $http.get('http://js-playground-gatekeeper.herokuapp.com/authenticate/' + code).success(init);
    };
    init = function(auth) {
      if (!auth) {
        return;
    ...
    loadGists = function() {
      return gh.github.getUser().gists(function(err, gists) {
        if (err) {
  ...
  App.factory('runner', function($rootScope) {
    var createWorker, print, tostr, worker;
    createWorker = function(scripts) {
      ...
      try {
        ...
        return alert("Blob worker scripts not supported");
      }
    };
    worker = {
      n: 0,
      ...
      send: function(o) {
        ...
        return this.curr.postMessage(o);
      },
      receieve: function(fn) {
        if (!this.curr) {
          this.start();
        }
  ...
  App.run(function($rootScope, gh, console) {
    ...
    return setTimeout(function() {
      var superkey;
      superkey = /Macintosh/.test(window.navigator.userAgent) ? "Cmd" : "Ctrl";
      return $.notify("Run with " + superkey + "+Enter", "info");
    }, 10 * 1000);
  });
